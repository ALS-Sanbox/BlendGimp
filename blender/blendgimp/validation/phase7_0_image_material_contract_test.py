...
# 0.5.3 release identity
check("manifest version 0.5.3", 'version = "0.5.3"' in manifest)
check("IPC version 0.5.3", 'BLENDGIMP_VERSION = "0.5.3"' in ipc)
check("usability build id", 'BUILD_ID = "7.0-image-material-ui"' in paint)
check("entry point identifies 0.5.3", '0.5.3 — Phase 7.0 Usability / Production UI' in init)
check("main panel identifies production UI", 'BlendGimp 0.5.3 — Production UI' in main)
