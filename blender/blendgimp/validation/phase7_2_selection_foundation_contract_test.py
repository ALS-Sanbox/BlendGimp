...
# 0.5.8 Phase 7.2 release identity
check("manifest version 0.5.8", 'version = "0.5.8"' in manifest)
check("IPC version 0.5.8", 'BLENDGIMP_VERSION = "0.5.8"' in ipc)
check("selection-foundation build id", 'BUILD_ID = "7.2-selection-foundation"' in paint)
check("entry point identifies 0.5.8", '0.5.8 — Phase 7.2 Selection Foundation' in init)
check("main panel identifies Phase 7.2", 'BlendGimp 0.5.8 — Phase 7.2 Selection Foundation' in main)
...
check("GIMP component version 0.5.8", 'BLENDGIMP_VERSION = "0.5.8"' in gimp)
...
# 0.5.8 color-tag UI visibility regression
