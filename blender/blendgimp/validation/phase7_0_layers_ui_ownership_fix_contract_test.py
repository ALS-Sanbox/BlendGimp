...
# 0.5.5 release identity
check("manifest version 0.5.5", 'version = "0.5.5"' in manifest)
check("IPC version 0.5.5", 'BLENDGIMP_VERSION = "0.5.5"' in ipc)
check("layers usability build id", 'BUILD_ID = "7.0-layers-ui-ownership-fix"' in paint)
check("entry point identifies 0.5.5", '0.5.5 — Phase 7.0 Usability / Production UI' in init)
check("main panel identifies production UI", 'BlendGimp 0.5.5 — Production UI' in main)
