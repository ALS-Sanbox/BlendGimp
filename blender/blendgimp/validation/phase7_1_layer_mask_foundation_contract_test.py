...
# 0.5.6 Phase 7.1 release identity
check("manifest version 0.5.6", 'version = "0.5.6"' in manifest)
check("IPC version 0.5.6", 'BLENDGIMP_VERSION = "0.5.6"' in ipc)
check("layer-mask foundation build id", 'BUILD_ID = "7.1-layer-mask-foundation"' in paint)
check("entry point identifies 0.5.6", '0.5.6 — Phase 7.1 Advanced Layers & Masks' in init)
check("main panel identifies Phase 7.1", 'BlendGimp 0.5.6 — Phase 7.1 Layers & Masks' in main)
...
check("GIMP component version 0.5.6", 'BLENDGIMP_VERSION = "0.5.6"' in gimp)
